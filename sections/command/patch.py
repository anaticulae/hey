...
def read_workplan(plan, inspace, outspace=None, verify: bool = False):
    ...
    for step in plan:
        ...
        inputs = recursive_inputs(inputs, inspace, outspace)
        ...
        ret += verify_resources(inputs, outputs, outspace)
...
def recursive_inputs(inputs, inspace, outspace):
    result = []
    for item in inputs:
        inpath = join(inspace, item)
        outpath = join(outspace, item)
        if exists(inpath):
            # normal input exists
            result.append(inpath)
        else:
            logging('Recursive input %s' % outpath)
            ...
            # else:
            #     # no input exists
            #     logging_error('Recursive input does not exists: %s' % outpath)
            #     result.append(inpath)
    return result


def verify_resources(inputs, outputs, outspace):
    ret = 0
    # require input files
    for path in inputs:
        if outspace in path:
